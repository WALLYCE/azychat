                    ...
                    >
                      <div class="full-width row col justify-between text-left q-pa-xs">
                        ...
                        <q-btn
                          ...
                          class="bg-padrao"
                          style="z-index: 999"
                          @click="removeConditionItem(node.id, condition, idx)"
                        />
                      </div>
                      ...
                      <q-card-section class="q-pa-sm">
                        ...
                        <div class="row q-mt-sm">
                          <div class="col">
                            <q-select
                              ...
                              emit-value
                              clearable
                              @input="nextStepId => addLineStep(nextStepId, idx, '#8db1dd', 'default')"
                            />
                            <q-select
                        ...
                        <div  v-if="condition.action === 4 || condition.action === 5 || condition.action === 6" class="row q-mt-sm">
                          <div class="col">
                            <div class="bg-light-green-5">
                                <div class="text-bold q-px-sm"> Resposta seja V Rotear para: </div>
                                <q-option-group
                                  class="text-center"
                                  inline
                                  v-model="condition.condition_v.action"
                                  :options="optionsAcao.filter(option => option.value <= 3)"
                                  color="primary"
                                  
                                />
                                <div class="col">
                            <q-select
                              v-if="condition.condition_v.action === 0"
                              dense
                              rounded
                              outlined
                              class="full-width"
                              :value="condition.condition_v.nextStepId || ''"
                              :options="nodesList.nodeList.filter(n => n.type !== 'configurations')"
                              option-label="name"
                              option-value="id"
                              label="Etapa"
                              ...
                              emit-value
                              clearable
                              @input="nextStepId => addLineStep(nextStepId, idx, '#00ff00', 'v')"
                            />
                            <q-select
                              v-if="condition.condition_v.action === 1"
                              dense
                              outlined
                              rounded
                              class="full-width"
                              v-model="condition.condition_v.queueId"
                              :options="filas"
                              option-label="queue"
                              option-value="id"
                              label="Fila"
                              :key="condition.condition_v.queueId"
                              map-options
                              emit-value
                              clearable
                              @input="condition.condition_v.nextStepId = null; condition.condition_v.userIdDestination = null"
                            />
                            <q-select
                              v-if="condition.condition_v.action === 2"
                              dense
                              outlined
                              rounded
                              class="full-width"
                              v-model="condition.condition_v.userIdDestination"
                              :options="usuarios"
                              option-label="name"
                              option-value="id"
                              label="Usuário"
                              ...
                              emit-value
                              clearable
                              @input="condition.condition_v.nextStepId = null; condition.condition_v.queueId = null"
                            />
                          </div>
                                
                          </div>
                            <div class="bg-red-5"> 
                              <div class="text-bold q-px-sm"> Resposta seja F Rotear para: </div>
                              <q-option-group
                                class="text-center"
                                inline
                                v-model="condition.condition_f.action"
                                :options="optionsAcao.filter(option => option.value <= 3)"
                                color="primary"
                              />
                              <div class="col">
                                <q-select
                              v-if="condition.condition_f.action === 0"
                              dense
                              rounded
                              outlined
                              class="full-width"
                              :value="condition.condition_f.nextStepId"
                              :options="nodesList.nodeList.filter(n => n.type !== 'configurations')"
                              option-label="name"
                              option-value="id"
                              label="Etapa"
                              ...
                              emit-value
                              clearable
                              @input="nextStepId => addLineStep(nextStepId, idx, '#00ff00', 'f')"
                            />
                            <q-select
                              v-if="condition.condition_f.action === 1"
                              dense
                              outlined
                              rounded
                              class="full-width"
                              v-model="condition.condition_f.queueId"
                              :options="filas"
                              option-label="queue"
                              option-value="id"
                              label="Fila"
                              :key="condition.condition_f.queueId"
                              map-options
                              emit-value
                              clearable
                              @input="condition.condition_f.nextStepId = null; condition.condition_f.userIdDestination = null"
                            />
                            <q-select
                              v-if="condition.condition_f.action === 2"
                              dense
                              outlined
                              rounded
                              class="full-width"
                              v-model="condition.condition_f.userIdDestination"
                              :options="usuarios"
                              option-label="name"
                              option-value="id"
                              label="Usuário"
                              ...
                              emit-value
                              clearable
                              @input="condition.condition_f.nextStepId = null; condition.condition_f.queueId = null"
                            />
                          </div>
                          </div>
                          </div>
  ...
  data () {
    return {
      ...
      optionsAcao: [
        ...
        { value: 1, label: 'Fila' },
        { value: 2, label: 'Usuário' },
        { value: 3, label: 'Finalizar Atendimento' },
        { value: 4, label: 'Hubsoft - Fatura a vencer'},
        { value: 5, label: 'Hubsoft - Fatura vencida' },
        { value: 6, label: 'Hubsoft - Desbloqueio de Confiança' },
  ...
  methods: {
    ...
    addCondiction () {
      this.node.conditions.push({
        ...
        id: this.gerarUID(),
        condition_f: {
          queueId: null, // Se necessário
          nextStepId: null, // Se necessário
          userIdDestination: null // Se necessário
        },
        condition_v: {
          queueId: null, // Se necessário
          nextStepId: null, // Se necessário
    ...
    removeConditionItem (id, condition, idx) {
      this.$q.dialog({
        title: 'Atenção!!',
        ...
        this.$emit('deleteLineAux', id, condition.nextStepId)
        this.$emit('deleteLineAux', id, condition.condition_f.nextStepId)
        this.$emit('deleteLineAux', id, condition.condition_v.nextStepId)
    ...
    addLineStep (nextStepId, idx, color, type) {
      if (type == 'default') {
      ...
      if (oldToLine != nextStepId) {
        this.$emit('addNewLineCondition', this.node.id, nextStepId, oldToLine, color)
      }
    } else if (type == 'v') {
      if (this.node.conditions[idx]?.condition_v.queueId) {
        this.node.conditions[idx].condition_v.queueId = null
      }
      if (this.node.conditions[idx]?.condition_v.userIdDestination) {
        this.node.conditions[idx].condition_v.userIdDestination = null
      }
      const oldToLine = this.node.conditions[idx].condition_v.nextStepId
      this.node.conditions[idx].condition_v.nextStepId = nextStepId
      if (oldToLine != nextStepId) {
        this.$emit('addNewLineCondition', this.node.id, nextStepId, oldToLine, color)
      }
    }
    else{
      if (this.node.conditions[idx]?.condition_f.queueId) {
        this.node.conditions[idx].condition_f.queueId = null
      }
      if (this.node.conditions[idx]?.condition_f.userIdDestination) {
        this.node.conditions[idx].condition_f.userIdDestination = null
      }
      const oldToLine = this.node.conditions[idx].condition_f.nextStepId
      this.node.conditions[idx].condition_f.nextStepId = nextStepId
      if (oldToLine != nextStepId) {
        this.$emit('addNewLineCondition', this.node.id, nextStepId, oldToLine, color)
      }
    }
