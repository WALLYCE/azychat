...
export const initIO = (httpServer: Server): SocketIO => {
  ...
  let usersOnline: { idUser: string }[] = [];

  io.on("connection", socket => {
    const { tenantId, id } = socket.handshake.auth;
    if (tenantId) {
      ...
      socket.on('Im-Online', data =>{
        if (!usersOnline.some(user => user.idUser === data)) {
          usersOnline.push({idUser: data});
    ...
    socket.on("disconnect", (reason: any) => {
      usersOnline = usersOnline.filter(user => user.idUser !== id);
      logger.info({
